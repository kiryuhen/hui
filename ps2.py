...
def get_stats(period_days):
    ...
    plt.plot(timestamps, temps, label='Температура (°C)')
    plt.plot(timestamps, hums, label='Влажность (%)')
    plt.plot(timestamps, press, label='Давление (hPa)')
    plt.legend()
    plt.title(f'Данные за {period_days} дней')
    plt.xlabel('Время')
    plt.ylabel('Значения')
    buf = BytesIO()
...
async def week_stats(callback: types.CallbackQuery):
    if callback.from_user.id != USER_ID:
        await callback.answer("Доступ запрещен.")
        return
    stats, img = get_stats(7)
    await callback.message.answer(stats)
    if img:
        await callback.message.answer_photo(photo=types.BufferedInputFile(img.read(), filename="week.png"))
...
async def month_stats(callback: types.CallbackQuery):
    if callback.from_user.id != USER_ID:
        await callback.answer("Доступ запрещен.")
        return
    stats, img = get_stats(30)
    await callback.message.answer(stats)
    if img:
        await callback.message.answer_photo(photo=types.BufferedInputFile(img.read(), filename="month.png"))
